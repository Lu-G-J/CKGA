...
class INTERGCN_BERT(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # changed
        encoder_layer, pooled_output = self.bert(text_bert_indices, token_type_ids=bert_segments_ids, return_dict=False)
        
        
        text_out = encoder_layer
        
        ######## aspect_emb
        aspect_emb = torch.sum(self.mask(text_out, aspect_double_idx), dim=1) / aspect_len.unsqueeze(-1)
        ########
        #####################
        ...
        output = self.fc(x)
        ##################### changed
        return {'emb':x, 'classification':output, 'aspect_emb':aspect_emb}
        #####################
